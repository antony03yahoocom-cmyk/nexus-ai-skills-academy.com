import { BookOpen, Users, DollarSign, TrendingUp, Award, FolderOpen, FileText } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const { data: purchases = [] } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.from("course_purchases").select("*").eq("status", "paid"); return data ?? []; },
  });
  ...
  const paidCount = profiles.filter((p: any) => p.is_premium || p.subscription_status === "paid").length;
  ...
  const stats = [
    ...
    { label: "Purchases", value: String(purchases.length), icon: DollarSign, color: "text-success", bg: "bg-success/10" },
  ];
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {pendingSubmissions > 0 && (
              <Link to="/admin/courses" className="glass-card p-5 border-accent/30 hover:border-accent/50 transition-all flex items-center gap-4">
