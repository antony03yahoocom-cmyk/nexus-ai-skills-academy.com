...
const LoginPage = () => {
  ...
  const handleGoogleSignIn = async () => { const result = await lovable.auth.signInWithOAuth("google", { redirect_uri: `${window.location.origin}/dashboard`, extraParams: { prompt: "select_account" } }); if (result.error) toast.error((result.error as any).message || "Google sign-in failed"); else if (!result.redirected) navigate("/dashboard"); };
