...
const AdminCoursesManager = () => {
  ...
  const { data: courses = [] } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.from("courses").select("*").order("created_at", { ascending: false }).limit(100); return data ?? []; },
  });

  const { data: modules = [] } = useQuery({
    queryKey: ["admin-modules"],
    queryFn: async () => { const { data } = await supabase.from("modules").select("*").order("sort_order").limit(500); return data ?? []; },
  });

  const { data: lessons = [] } = useQuery({
    queryKey: ["admin-lessons"],
    queryFn: async () => { const { data } = await supabase.from("lessons").select("*").order("sort_order").limit(1000); return data ?? []; },
  });

  const { data: assignments = [] } = useQuery({
    queryKey: ["admin-assignments"],
    queryFn: async () => { const { data } = await supabase.from("assignments").select("*").limit(1000); return data ?? []; },
  });

  const modulesByCourse = useMemo(() => {
    const grouped = new Map<string, any[]>();
    modules.forEach((module: any) => {
      const items = grouped.get(module.course_id) ?? [];
      items.push(module);
      grouped.set(module.course_id, items);
    });
    return grouped;
  }, [modules]);
  ...
  const lessonsByModule = useMemo(() => {
    ...
    lessons.forEach((lesson: any) => {
      const items = grouped.get(lesson.module_id) ?? [];
      items.push(lesson);
      grouped.set(lesson.module_id, items);
    });
    return grouped;
  }, [lessons]);
  ...
  const assignmentsByLesson = useMemo(() => {
    ...
    assignments.forEach((assignment: any) => {
      const items = grouped.get(assignment.lesson_id) ?? [];
      items.push(assignment);
      grouped.set(assignment.lesson_id, items);
    });
    return grouped;
  }, [assignments]);
  ...
  const createModule = useMutation({
    mutationFn: async () => {
      const courseModules = modulesByCourse.get(moduleForm.course_id) ?? [];
  ...
  const createLesson = useMutation({
    mutationFn: async (fileUrl?: string) => {
      const moduleLessons = lessonsByModule.get(lessonForm.module_id) ?? [];
                            ...
                            {isModExpanded && (
                              <div className="px-3 pb-3 space-y-2">
                                {modLessons.map((lesson: any) => {
                                  const lessonAssigns = assignmentsByLesson.get(lesson.id) ?? [];
